...
import { rest, type ProductImage } from './shopify.ts';
...
async function findId(listPath: string, key: string, handle: string): Promise<number | null> {
...
async function main(): Promise<void> {
  ...
  for (const [handle, ids] of Object.entries(productImages)) {
    ...
    const cur = await rest<{ images: ProductImage[] }>('GET', `/products/${id}/images.json`);
    for (const img of cur.images || []) await rest('DELETE', `/products/${id}/images/${img.id}.json`);
    // add real ones (first = featured)
    for (let i = 0; i < ids.length; i++) {
      await rest('POST', `/products/${id}/images.json`, {
        image: { src: px(ids[i]), position: i + 1, alt: handle.replace(/-/g, ' ') },
      });
    }
